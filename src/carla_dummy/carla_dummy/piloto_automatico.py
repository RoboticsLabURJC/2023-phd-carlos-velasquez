...
# Configurar carpeta de salida y archivo CSV
output_folder = "/home/canveo/carla_ws/dataset/images"
csv_file = "/home/canveo/carla_ws/dataset/data.csv"
...
with open(csv_file, mode='w', newline='') as file:
    ...
    writer.writerow(['image_name', 'steer', 'throttle', 'brake', 'speed'])
...
def image_callback(image):
    ...
    for i, line in enumerate(info_text):
        y = y0 + i*dy
        cv2.putText(frame, line, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)  # display_frame

    # Display the frame
    cv2.imshow("CARLA Manual Control", frame)   # display_frame

    # Save image without text and log data
    image_name = f"frame{int(current_time*1000)}.png"
    image_path = os.path.join(output_folder, image_name)
    cv2.imwrite(image_path, frame)  # Save the image without the overlay text

    with open(csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([image_name, control.steer, control.throttle, control.brake, speed])
