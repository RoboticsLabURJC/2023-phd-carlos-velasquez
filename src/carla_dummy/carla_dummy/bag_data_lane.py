...
class DataRecorder(Node):
    def __init__(self):
        ...
        self.subscription_curvature = self.create_subscription(
            String,
            '/curvature_status',
        ...
        self.output_curvature_file = os.path.join(self.path_output, 'curvature_data.npy')
    ...
    def curvature_callback(self, msg):
        ...
        timestamp = self.get_clock().now().nanoseconds
        self.curvature_timestamps.append(timestamp)
        self.curvature_values.append(msg.data)
    ...
    def save_data(self):
        ...
        np.save(self.output_curvature_file, {'timestamps': self.curvature_timestamps, 'curvature': self.curvature_values})
        ...
        curvature_data = np.load(self.output_curvature_file, allow_pickle=True).item()
        ...
        curvature_df = pd.DataFrame({'timestamp': curvature_data['timestamps'], 'curvature': curvature_data['curvature']})

        merged_df = pd.merge_asof(image_df, control_df, on='timestamp', direction='forward')
        merged_df = pd.merge_asof(merged_df, curvature_df, on='timestamp', direction='forward')
