...
class VehicleTeleop(Node):
    def __init__(self):
        ...
        self.image_subscriber = self.create_subscription(
            Image, "/carla/ego_vehicle/rgb_view/image", self.third_person_image_cb, 10)
        self.image_subscriber = self.create_subscription(
            Image, "/carla/ego_vehicle/rgb_front/image", self.first_person_image_cb, 10)
        ...
        self.vehicle_control_manual_override_publisher = self.create_publisher(
            Bool, "/carla/ego_vehicle/vehicle_control_manual_override", 10)
        self.auto_pilot_enable_publisher = self.create_publisher(
    ...
    def pipeline(self, img, s_thresh=(100, 255), sx_thresh=(15, 255)):
        ...
        color_binary = numpy.dstack(
            (numpy.zeros_like(sxbinary), sxbinary, s_binary)) * 255
    ...
    def sliding_window(self, img, nwindows=9, margin=150, minpix=1, draw_windows=True):
        ...
        for window in range(nwindows):
            ...
            if len(good_left_inds) > minpix:
                leftx_current = int(numpy.mean(nonzerox[good_left_inds]))
            if len(good_right_inds) > minpix:
                rightx_current = int(
                    numpy.mean(nonzerox[good_right_inds]))
    ...
    def get_curve(sef, img, leftx, rightx):
        ...
        left_curverad = ((1 + (2*left_fit_cr[0]*y_eval*ym_per_pix +
                         left_fit_cr[1])**2)**1.5) / numpy.absolute(2*left_fit_cr[0])
        right_curverad = (
            (1 + (2*right_fit_cr[0]*y_eval*ym_per_pix + right_fit_cr[1])**2)**1.5) / numpy.absolute(2*right_fit_cr[0])
    ...
    def draw_lanes(self, img, left_fit, right_fit):
        ...
        right = numpy.array(
            [numpy.flipud(numpy.transpose(numpy.vstack([right_fit, ploty])))])
