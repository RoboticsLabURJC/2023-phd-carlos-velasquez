...
class LaneDetector:
    def __init__(self):
        self.camera_image = np.zeros(shape=(640, 480, 3), dtype=np.uint8)
    ...
    def detect_lane(self, image):
        ...
        size_im = cv2.resize(self.camera_image, dsize=(640, 480))  # VGA resolution
      
        roi = size_im[320:480, 95:500]  # [380:430, 330:670]   [y:y+b, x:x+a]
        roi_im = cv2.resize(roi, (405, 160))  # (a of x, b of y)
        ...
        # a_avg_le = ((y2_avg_le - y1_avg_le) / (x2_avg_le - x1_avg_le)) # nuevo
        if (x2_avg_le - x1_avg_le) != 0:
        ...
        #################################################
        FCP_img = np.zeros(shape=(480, 640, 3), dtype=np.uint8) + 0     # changed from (480, 640, 3)

        # FCP = np.array([pt1_avg_ri, pt2_avg_ri, pt1_avg_le, pt2_avg_le])
        # FCP = np.array([(100,100), (100,200), (200,200), (200,100)])
        FCP = np.array([pt2_avg_le, pt1_fi_le, pt2_fi_ri, pt1_avg_ri])
        cv2.fillConvexPoly(FCP_img, FCP, color=(115, 115, 115))  # BGR
        alpha = 0.9
        ...
        if 0 < lane_center_x <= 305:  ## 318
            cv2.putText(size_im, text_left, org, font, 0.7, (255, 0, 255), 2)
        elif 35 < lane_center_x < 335:  # 31,322
            # elif lane_center_x > 318 and lane_center_x < 322 :
            cv2.putText(size_im, text_center, org, font, 0.5, (255, 0, 255), 2)
        elif lane_center_x >= 335: # 322
            cv2.putText(size_im, text_right, org, font, 0.5, (255, 0, 255), 2)
        elif lane_center_x == 0:
            cv2.putText(size_im, text_non, org, font, 0.5, (255, 0, 255), 2)
        #################################################
