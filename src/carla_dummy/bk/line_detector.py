...
class VehicleTeleop(Node):
    ...
    def sliding_window(self, img, nwindows=9, margin=150, minpix = 1, draw_windows=True):
        ...
        for window in range(nwindows):
            ...
            if len(good_right_inds) > minpix:        
                rightx_current = int(numpy.mean(nonzerox[good_right_inds]))
    ...
    #            leftx_current = numpy.int(numpy.mean([numpy.mean(nonzerox[good_right_inds]) -900, leftx_current]))
        ...
        self.get_curve(img, leftx, rightx)
    ...
    def draw_lanes(self,img, left_fit, right_fit):
        ...
        points_list = [points[i,:,:] for i in range(points.shape[0])]
        points_list_int = [points.astype(numpy.int32) for points in points_list]
        
        cv2.fillPoly(color_img, points_list_int, (255,0,0))
