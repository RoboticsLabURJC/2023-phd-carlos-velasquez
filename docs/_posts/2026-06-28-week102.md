...
| **Test 2** | Diferentes puntos de inicio | **3 posiciones iniciales diferentes** (`random_1`, `random_2` y `random_3`), con **75 ejecuciones por posición** (**225 ejecuciones** en total). La trayectoria fue corregida para que todas las condiciones completen una vuelta del mismo circuito; únicamente cambia el punto de inicio. La meta se ubica aproximadamente 10–15 m antes del punto de partida, siguiendo el sentido de circulación. |
...
| Test 2 | Posiciones de inicio random   | 3 posiciones de inicio seleccionadas en sectores distintos del circuito. La trayectoria ideal se reorganiza circularmente para cada posición, de modo que las tres condiciones completan el mismo circuito y solo cambia el punto de inicio. La meta se sitúa aproximadamente 10–15 m antes del punto de partida, siguiendo el sentido de circulación. |
...
El Test 2 fue corregido para que las tres condiciones de evaluación completen el mismo circuito, modificando únicamente el punto de inicio del vehículo. En la implementación actual se utilizan tres posiciones de arranque diferentes (`random_1`, `random_2` y `random_3`). Para cada una, la trayectoria ideal se reorganiza circularmente a partir del punto seleccionado, de manera que el vehículo recorre una vuelta completa antes de alcanzar la meta.

En la versión inicial del test, las distintas posiciones de inicio producían recorridos de diferente longitud y, por tanto, condiciones de evaluación no equivalentes. La corrección elimina esta diferencia: ahora todas las configuraciones cubren la misma trayectoria completa y se distinguen exclusivamente por el sector del circuito desde el cual comienza la conducción. Por esta razón, los resultados obtenidos con la metodología anterior no son directamente comparables con la campaña actual.

Los resultados disponibles muestran que las mezclas DAgger con proporciones entre 30 % y 75 % presentan las mayores tasas de finalización. Tanto $D_{0.50}^{B}$ como $D_{0.75}^{B}$ alcanzan 99.1 % de éxito (223/225), seguidas por $D_{0.30}^{B}$ con 98.2 % (221/225). Entre ellas, $D_{0.50}^{B}$ ofrece el mejor equilibrio: completa prácticamente todas las ejecuciones, no registra colisiones y mantiene las invasiones de carril en 2.327. En cambio, aumentar la proporción de DAgger hasta 75 % no mejora la tasa de éxito y eleva las colisiones, las invasiones de carril y la desviación media. Esto indica que, para este escenario, una mayor cantidad de muestras de recuperación no se traduce necesariamente en una trayectoria más estable.

La configuración $D_{0.15}^{B}$ se aparta de esta tendencia. Aunque alcanza la mayor velocidad media del test (55.70 km/h) y no presenta colisiones, su tasa de éxito disminuye a 90.2 %, mientras que las invasiones de carril aumentan hasta 10.182 y la desviación media alcanza 0.510 m. Por tanto, la mayor velocidad no representa una mejora global, sino que coincide con un seguimiento lateral menos preciso.

En Noise Injection, $D_{0.15}^{C}$ obtiene el mejor compromiso entre finalización y precisión: alcanza 98.2 % de éxito (221/225), registra 0.032 colisiones, 2.000 invasiones de carril y presenta la menor desviación media de todas las configuraciones evaluadas (0.255 m). Al incrementar la proporción de Noise Injection a 30 % y 50 %, la desviación y las invasiones permanecen bajas en los episodios exitosos, pero la tasa de finalización cae hasta 72.0 % y 72.9 %, respectivamente. Esto sugiere que dichas políticas conducen de forma precisa cuando logran completar el circuito, aunque presentan menor consistencia para finalizarlo desde los tres puntos de inicio.

En conjunto, los datos disponibles sugieren dos comportamientos complementarios: las mezclas DAgger de 30–50 % favorecen la finalización del recorrido desde diferentes sectores del mapa, mientras que Noise Injection, especialmente en 15 %, favorece un seguimiento lateral más preciso. No obstante, esta interpretación es todavía preliminar. El modelo base $D^{A}$ aún no cuenta con ejecuciones válidas y tampoco se dispone de resultados para todas las proporciones de Noise Injection, por lo que no es posible cuantificar la mejora de cada estrategia respecto a Burbuja ni establecer una configuración óptima definitiva. Además, dado que las métricas de desempeño se calculan únicamente sobre episodios exitosos, los bajos valores de desviación de $D_{0.30}^{C}$ y $D_{0.50}^{C}$ deben interpretarse junto con sus menores tasas de finalización.
